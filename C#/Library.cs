...
namespace Library
{
	...
	public class ViewClass
	{
			...
			while (true)
			{
				...
				}))
				{
					case 0:
						...
						var reader = Search("Выберите читателя. Введите id, Имя Фамилию, номер телефона, адрес читателя и до какого чила нужно вернуть книгу ",
							controller.FindReaders, DisplayReaderShort);
						if (reader == null) break;
						Console.WriteLine("Введите дату когда нужно вернуть:");
						...
						DateTime date_return = new DateTime(year, month, day);
						controller.RentBook(copy.ItemId, reader.ItemId, date_return);
					...
					case 1:
						...
						switch (retcopyid)
						{
							...
							default:
								...
								if(DateTime.Now > retcopy.DateReturn)
									Console.WriteLine("Возврат задержан, полагается штраф: ");
		...
		protected string DisplayReader(ModelClass.Reader reader)
		{
			...
			foreach (var copyid in reader.OnHand)
			{
				...
				s.AppendLine(copy.DateTaken.ToString() + "->" + copy.DateReturn.ToString() + " " + DisplayBookShort(controller.GetBook(copy.Book)));
			}
			return s.ToString();
		}
		...
		protected string DisplayCopy(ModelClass.Copy copy)
		{
			...
			if (copy.OnHand != 0)
			{
				...
				s.AppendLine("Вернуть: " + copy.DateReturn.ToString());
			}
			else
	...
	public class ControllerClass
	{
		...
		public void RentBook(int copyid, int readerid, DateTime date_return)
		{
			var copy = GetCopy(copyid);
			if (copy == null) return;
